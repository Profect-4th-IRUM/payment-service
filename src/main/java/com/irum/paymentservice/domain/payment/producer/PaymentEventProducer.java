...
public class PaymentEventProducer {
    ...
    private final KafkaTemplate<String, PaymentPaidEvent> kafkaTemplate;
    ...
    public void sendPaymentPaidEvent(OrderStatus orderStatus, UUID orderId){
        ...
        ProducerRecord<String, PaymentPaidEvent> record = new ProducerRecord<>(kafkaTopicProperties.paymentPaid(), key, event);
        ...
        try {
            kafkaTemplate.send(record).whenComplete((result, exception) -> {
                if (exception == null) {
                    ...
                    //실패 처리
                    log.error("[Error] sending payment paid event : {}", exception.getMessage(), exception);
                }
            });
        } catch (Exception e) {
