...
public class PaymentService {
    ...
    public PaymentResponse createPayment(PaymentRequest request) {
        Payment payment = paymentRepository.findById(request.paymentId()).orElseThrow(
                () -> new CommonException(PaymentErrorCode.PAYMENT_ERROR)
        );
        ...
        try {
            ...
            String OrderNum = orderClient.updateOrderStatusPreparing(OrderStatus.PREPARING, request.orderId());
            ...
            orderClient.updateOrderStatusFailed(OrderStatus.FAILED, request.orderId(), request.paymentId();
